...
- In the begining data stored on physical server, but through the time the amount of data was growing more and more. So the physical storages needed to be more and more, it was expensive and costed a lot of time to install a new server. So they came up with hypervisors which made it possible to split a single device into several parts. This reduced expenditure on physical storage media. But it was not enough. Now they figured out the access to these several parts can be online in the Internet and now you are able to just rent some space in online storage. But still it was a problem in operation system that launched on each several part, it costed resources and time. So they figured out that the several parts can use the same one operation system that the host has. It solved operating system overhead and environment inconsistency.
